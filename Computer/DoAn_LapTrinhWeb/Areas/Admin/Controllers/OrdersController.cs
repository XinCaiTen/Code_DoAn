...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class OrdersController : BaseController
    {
        ...
        //hủy đơn hàng
        public ActionResult OrderDelTrash(int? id,string returnUrl)
        {
            if (String.IsNullOrEmpty(returnUrl) && Request.UrlReferrer != null && Request.UrlReferrer.ToString().Length > 0)
            {
                return RedirectToAction("OrderDelTrash", new { returnUrl = Request.UrlReferrer.ToString() });
            }
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                if (order == null)
                {
                    Notification.set_flash("Đơn hàng: " + "NVT" + id + " đã được hoàn thành, không thể thay đổi trạng thái khác!", "warning");
                    return RedirectToAction("OrderIndex");
                }
                else
                {
        ...
        [HttpPost]
        [ActionName("OrderDelTrash")]
        [ValidateAntiForgeryToken]
        public ActionResult DelTrashConfirmed(int id, string Roles, string returnUrl)
        {
                ...
                if (order != null)
                {
                    ...
                    db.SaveChanges();
                    Notification.set_flash("Đã hủy đơn hàng: " + "NVT" + id, "success");
                ...
                {
                    Notification.set_flash("Đơn hàng: " + "NVT" + id + " đã được hoàn thành, không thể hủy!", "warning");
                }
            return View(order);
        }
        ...
        public ActionResult ChangeProcessing(int? id,string Roles)
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                if (order != null)
                {
                    ...
                    db.SaveChanges();
                    Notification.set_flash("Đã chuyển trạng thái đơn hàng: "+"NVT"+id+" sang đang xử lý!", "success");
                }
                else
                {
                    Notification.set_flash("Đơn hàng: "+"NVT"+id+" đã được hoàn thành, không thể thay đổi trạng thái khác!", "warning");
        ...
        public ActionResult ChangeComplete(int? id,string Roles)
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                db.SaveChanges();
                Notification.set_flash("Đã chuyển trạng thái đơn hàng: "+"NVT"+id+" sang Hoàn thành!", "success");
                return RedirectToAction("OrderIndex");
            }
            else
            {
