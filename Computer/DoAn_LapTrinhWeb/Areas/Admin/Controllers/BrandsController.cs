...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class BrandsController : BaseController
    {
        ...
        //Xóa thương hiệu
        public ActionResult BrandDelete(int? id, string returnUrl)
        {
            if (String.IsNullOrEmpty(returnUrl) && Request.UrlReferrer != null && Request.UrlReferrer.ToString().Length > 0)
            {
                return RedirectToAction("BrandDelete", new { returnUrl = Request.UrlReferrer.ToString() });
            }
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                var brand = _db.Brands.SingleOrDefault(pro => pro.brand_id == id);
        ...
        //Xác nhận xóa vĩnh viễn thương hiệu
        [HttpPost]
        [ActionName("BrandDelete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id, string returnUrl)
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                _db.SaveChanges();
                Notification.set_flash("Đã xoá vĩnh viễn thương hiệu: " + brand.brand_name + "", "success");
                if (!String.IsNullOrEmpty(returnUrl))
                    return Redirect(returnUrl);
                else
                    return RedirectToAction("BrandIndex");
