...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class RolesController : Controller
    {
        ...
        //Xóa tài khoản
        public ActionResult RolesDelete(int? id, string returnUrl)
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                if (String.IsNullOrEmpty(returnUrl) && Request.UrlReferrer != null && Request.UrlReferrer.ToString().Length > 0)
                {
                    return RedirectToAction("RolesDelete", new { returnUrl = Request.UrlReferrer.ToString() });
                }
                var account = db.Accounts.SingleOrDefault(a => a.account_id == id);
                if (account == null || id == null)
                {
        ...
        public ActionResult DeleteConfirmed(int id, string returnUrl)
        {
            if (User.Identity.GetRole() == "0" || User.Identity.GetRole() == "2")
            {
                ...
                db.SaveChanges();
                Notification.set_flash("Đã xoá vĩnh viễn tài khoản: " + account.Email + "", "success");
